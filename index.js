...
function CemberinCevresi(yariCap) {
  /* kodlar buraya */
  const cemberinCevresi = 2 * pi * yariCap;
...
function CemberinAlani(yariCap) {
  const cemberinAlani = yariCap * yariCap * pi;
...
/* 	GÖREV 3:
  - Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
    ...
    3f. `tekraredensayilar` adında bir dizi oluşturun. sayilar dizisi içerisindeki bazı sayılar birden fazla kere yazılmış. sayilar dizisi içerisinde birden fazla kez yazılmış sayıları tespit ederek kaç kere tekrar edildiğini belirten bir string oluşturulup 
...
// ucetambolunenler arrayini olusturuyorum
...
// ucetambolunenler arrayini reduce ile nesneleri toplayacagim
function topla(total, number) {
  return total + number;
}
ucebolunenlerintoplami = ucetambolunenler.reduce(topla, 0);

besyuzdenkucuksayilar = sayilar.filter((item) => item < 500);

siralisayilar = besyuzdenkucuksayilar.sort((a, b) => a - b);

let kontrolData = {};   // { 1: 5, 2:3}
for (let i = 0; i < sayilar.length; i++) {
  if (kontrolData[sayilar[i]]) {
    kontrolData[sayilar[i]] += 1;
  } else {
    kontrolData[sayilar[i]] = 1;
  }
}
for (let key in kontrolData) {
  if (kontrolData[key] > 1) {
    tekraredensayilar.push(`${key} sayısı ${kontrolData[key]} kere tekrar edilmiştir`);
  }
}
